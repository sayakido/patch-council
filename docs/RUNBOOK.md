...
这些命令只检查历史 Python 原型。当前产品主路径是 Workbench。

## Council Smoke Test (Python 原型，仅参考)
...
## Design Council Smoke Test (Node 全栈)

Workbench 中创建 `mode=design_council` session 后，预期流程：

```text
brainstorming_started
-> brainstorming_question_created 或 design_file_written
-> [需要用户回答时] brainstorming_answer_received
